...
This guide can be used as a GitHub learning guide and as supporting material for an Oracle ACE Apprentice product usage milestone.
...
## Beginner Explanation: What Is a Vector?
...
## Beginner Explanation: What Do the 5 Vector Dimensions Mean?
...
## Beginner Explanation: What Is Vector Search?
...
## Beginner Explanation: What Is Cosine Distance?
...
| Client Tool | SQL*Plus / SQLcl / SQL Developer |
...
│   ├── 05_sample_records_inserted.png
│   ├── 06_similarity_search_backup.png
│   ├── 07_similarity_search_network.png
│   └── 08_hnsw_index_created.png
└── docs/
    └── learning-notes.md
```
...
5. From **Privileged Operating System Groups**, keep the default values unless your organization requires custom group separation.
...
```text
screenshots/02_pdb_open_state.png
```
...
```text
screenshots/03_vector_lab_user_created.png
```
...
## Beginner Explanation: `VECTOR(5, FLOAT32)`
...
```text
screenshots/04_vector_table_created.png
```
...
## Beginner Explanation: `TO_VECTOR`
...
```text
screenshots/05_sample_records_inserted.png
```
...
## Beginner Explanation: `VECTOR_DISTANCE`
...
```text
screenshots/06_similarity_search_backup.png
```
...
```text
screenshots/07_similarity_search_network.png
```
...
## Beginner Explanation: `FROM_VECTOR`
...
## Beginner Explanation: What Is HNSW?
...
If index creation fails due to memory configuration, document the error and add the fix in your troubleshooting notes.
...
```text
screenshots/08_hnsw_index_created.png
```
...
Run only if you want to remove the lab objects.

```bash
sqlplus vector_lab/VectorLab_2026@KBPDB @sql/99_cleanup.sql
```

To drop the user completely, connect as `SYSDBA` inside the PDB and run:

```sql
DROP USER vector_lab CASCADE;
```
...
Capture the following screenshots for GitHub and ACE Apprentice product usage evidence.

| Screenshot | Description |
|---|---|
| `01_database_version.png` | Output of `SELECT banner_full FROM v$version;` |
| `02_pdb_open_state.png` | PDB opened and saved |
| `03_vector_lab_user_created.png` | `VECTOR_LAB` user created |
| `04_vector_table_created.png` | `DESC support_notes` showing `VECTOR(5, FLOAT32)` |
| `05_sample_records_inserted.png` | Sample support notes inserted |
| `06_similarity_search_backup.png` | Backup/recovery similarity search result |
| `07_similarity_search_network.png` | Network/listener similarity search result |
| `08_hnsw_index_created.png` | HNSW vector index created and verified |

---

# Suggested ACE Apprentice Submission Description

This submission demonstrates hands-on product usage of Oracle AI Database 26ai by testing Oracle AI Vector Search in an on-premise single-instance database environment.

I created a pluggable database for the lab, configured a dedicated `VECTOR_LAB` schema, created a table using the native `VECTOR` data type, inserted sample technical support records with manually defined vector embeddings, and performed vector similarity searches using `VECTOR_DISTANCE`.

The lab also demonstrates how vector search can be combined with normal SQL filtering and how an HNSW vector index can be created for approximate similarity search. The repository includes setup instructions, SQL scripts, screenshots, and beginner-friendly explanations so that entry-level users can understand and reproduce the lab.
...
- Oracle `VECTOR_DISTANCE` SQL function: https://docs.oracle.com/en/database/oracle/oracle-database/26/sqlrf/vector_distance.html
- Oracle `TO_VECTOR` SQL function: https://docs.oracle.com/en/database/oracle/oracle-database/26/sqlrf/to_vector.html
- Oracle `FROM_VECTOR` SQL function: https://docs.oracle.com/en/database/oracle/oracle-database/26/sqlrf/from_vector.html
- Oracle HNSW vector index syntax: https://docs.oracle.com/en/database/oracle/oracle-database/26/vecse/hierarchical-navigable-small-world-index-syntax-and-parameters.html
- Oracle vector search parameters: https://docs.oracle.com/en/database/oracle/oracle-database/26/vecse/oracle-ai-vector-search-parameters.html
- Oracle Linux 9 requirements for Oracle AI Database 26ai: https://docs.oracle.com/en/database/oracle/oracle-database/26/lacli/supported-oracle-linux-9-distributions-for-x86-64.html
